    ...
    var actions = {
      ...
      message : function(args){
        ...
        if (args.type === 'MESSAGE'){
          ...
          if (args.data.user_token !== tlkio.user.token){
            // Removing all the html tags
            var message = args.data.body.trim().replace(/(<([^>]+)>)/ig,'');
            ...
            if (message.toLowerCase().indexOf('@'+tlkio.user.nickname.toLowerCase()) === 0){
              ...
              if (commands.length > 0){
                if (commands[1] !== undefined){
                  ...
                  // Invoke the command
                  tlkioClient.emit('command__'+keyCommand, {text : message, html : args.data.body, fromUser : args.data.user, commands : commands});
                }
              }
            }
          }
        ...
        tlkioClient.emit(args.type.toLowerCase(), args);
      }
    };
